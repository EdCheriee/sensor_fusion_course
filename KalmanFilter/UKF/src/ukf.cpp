...
void UKF::PredictRadarMeasurement(VectorXd* z_out, MatrixXd* S_out) {
  ...
  for(auto i = 0; i < Xsig_pred.cols(); i++)
  {
    auto px  = Xsig_pred(0, i);
    auto py  = Xsig_pred(1, i);
    auto v   = Xsig_pred(2, i);
    auto psi = Xsig_pred(3, i);
...
void UKF::PredictMeanAndCovariance(VectorXd* x_out, MatrixXd* P_out) {
  ...
  x.fill(0.0);
  for(auto j = 0; j < Xsig_pred.cols(); j++)
  {
    x += weights(j) * Xsig_pred.col(j);
  }
  ...
  for(auto j = 0; j < Xsig_pred.cols(); j++)
  {
    VectorXd x_diff = Xsig_pred.col(j) - x;
    // angle normalization
    while (x_diff(3) > M_PI) 
      x_diff(3)-=2.*M_PI;
    while (x_diff(3) < -M_PI) 
    ...
    P += weights(j) * x_diff * x_diff.transpose();
